(function() {
  'use strict';
  var Datom, GUY, MAIN, alert, assign, debug, echo, get_base_types, help, info, inspect, letsfreezethat, letsfreezethat_nofreeze, log, matcher_cache, minimatch, plain, praise, rpr, urge, warn, whisper,
    indexOf = [].indexOf;
  ...
  Datom = class Datom {
    ...
    //---------------------------------------------------------------------------------------------------------
    _get_matchers(selectors) {
      var i, len, results, selector;
      results = [];
      for (i = 0, len = selectors.length; i < len; i++) {
        selector = selectors[i];
        results.push(this._get_matcher(selector));
      }
      return results;
    }
    ...
    select(d, ...selectors) {
      ...
      ref1 = this._get_matchers(selectors);
