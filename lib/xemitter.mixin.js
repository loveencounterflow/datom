  ...
  provide_library = function() {
    ...
    this.contract = function(key, self, listener) {
      ...
      return this._emitter.on(key, async(d) => {
        var R;
        R = (await listener.call(self, d));
        debug('^3446^', R, DATOM.new_datom('^xxx', R));
        debug('^3446^', R, this._mark_as_primary(R));
        return this._mark_as_primary(R);
      });
    };
    // @_mark_as_primary await listener.call self, d
